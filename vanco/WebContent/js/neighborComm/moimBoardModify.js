...
window.onload = function() {
	...
	xE.d.designMode = "on";
	xE.d.getElementsByTagName("body")[0].innerHTML = document.forms["fr"].content.value;
...
function deleteMyImageAction(filenum) {
	$.ajax({
		...
		success: function(result){
			...
			if(document.getElementById("file"+filenum))
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			for (var kk = 0; kk < fileObj.filesList.length; kk++) {
				...
				// getImagesByAlt(sel_files[i].name)[0]
