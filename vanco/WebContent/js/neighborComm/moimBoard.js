...
function handleFiles(files) {
	...
	if (!files.length) {
		...
		document.getElementById("filemsg").innerHTML = "사진이 없습니다.";
	...
	// 파일이 업로드 되었을 때
		...
		if (sumSize > 10485760) {
			alert("최대 첨부 용량을 초과하였습니다 \n 최대크기 : 10,485,760bytes 현재크기 : "+sumSize+"bytes");
...
function insertImageAction(index) {
	...
	// 로딩 화면 해제
	isloading.stop();
	realImgCss(); //로딩 끝나고 글쓰기 페이지 동적 css 적용하기
...
function realImgCss(){
	...
	/*var myimages = xE.d.getElementsByClassName("myimages");
	var divImg = xE.d.getElementsByClassName("divImg");
	
	for(var i=0;i<divImg.length;i++){
		divImg[i].style.width="500px";
	}
	...
	*/
	...
	//myimages.style.width="100px";
	
	//myimages.css('display','block').css('margin','0 auto').css('width','100px');
	//divImg.css('display','block').css('margin','0 auto').css('width','100px');
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			for (var kk = 0; kk < fileObj.filesList.length; kk++) {
				...
				getImagesByAlt(sel_files[kk].name)[0].src = "upload/" + fileObj.filesList[kk].filename;
